...
class OrderController extends Controller{
    ...
    public function tao_don(Request $request){
        try {
            // $params = $request->all();
            $params = [
            ...
            $check_phone = $this->helper->call_ntl_api_ntx('v1/web-checkExistUser', $s_phone, 'GET');
            $res_phone = json_decode($check_phone, true);

            $result = $this->helper->call_ntl_api_ntx('v1/push-RMQBill', $params, 'POST');
            $result = json_decode($result, true);
            return view('frontend.pages.thank_you')->with(compact('res_phone','result','params'));
    ...
    public function order(){
        return View('frontend.pages.dat_hang');
    }
    ...
    public function order_B1($id){
        return View('frontend.pages.dat_hang_B1',['id'=>$id]);
    }

    public function order_B2(){
        return View('frontend.pages.dat_hang_B2');
    }

    public function order_B3(Request $request){
        $data = $this->getProvince();
        return View('frontend.pages.dat_hang_B3',['data'=> $data]);
    }
    public function thanh_toan(){
        return View('frontend.pages.thanh_toan_van_don');
    }
    ...
    public function back_step(Request $req, $id, $step){
        ...
        $data['step1']['cod_amount'] =  $req->cod_amount;
        ...
        // Mã khuyến mãi
        $data['voucher'] = $req->voucher;
        // payment
        $data['payment_method_id'] = $req->payment_method_id;
        if ($step == 1) {
            return View('frontend.pages.dat_hang_B1',['data'=>$data],['id'=>$id]);
        } elseif ($step == 2) {
            return View('frontend.pages.dat_hang_B2',['data'=>$data],['id'=>$id]);
        } else {
            return View('frontend.pages.dat_hang_B3',['data'=>$data],['id'=>$id]);
        }
    }
    //kết thúc quay lại
    ...
    public function checkStep3(Request $request){
        $validation = Validator::make($request->all(),[
            'cargo_content' => ['required','regex:/[^~!@#$%^&*()_+{}:;""|<>?[]/'],
            'weight' => ['required','regex:/[^~!@#%^&*()_+{}:;""|<>?[a-zA-Z]$/'],
        ],[
            'cargo_content.regex' => 'Vui lòng nhập đúng',
            'cargo_content.required' => 'Vui lòng nhập hàng hóa',
            'weight.regex' => 'Vui lòng nhập đúng',
            'weight.required' => 'Vui lòng nhập khối lương',
        ]);
        if ($validation->passes()){
            return response()->json([
                'message' => 'Successful',
    ...
    //submit form
    public function step_post(Request $req, $id){
        // Mã khuyến mãi
        $data['voucher'] = $req->voucher;
        return View('frontend.pages.dat_hang_B1')->with(compact('data','id'));
    }
    public function step_1_post(Request $req, $id){
        $data['step2'] = [];
        $data['step2']['s_name'] =  $req->s_name;
        $data['step2']['s_phone'] =  $req->s_phone;
        $data['step2']['s_address'] =  $req->s_address;
        ...
        // Mã khuyến mãi
        $data['voucher'] = $req->voucher;
        return View('frontend.pages.dat_hang_B2')->with(compact('data','id'));    
    }

    public function step_2_post(Request $req, $id){
        $data['step2'] = [];
        $data['step2']['s_name'] =  $req->s_name;
        $data['step2']['s_phone'] =  $req->s_phone;
        $data['step2']['s_address'] =  $req->s_address;
        ...
        $data['step3']['chon_quan_noinhan'] =  $req->chon_quan_noinhan;
        $data['step3']['chon_phuong_noinhan'] =  $req->chon_phuong_noinhan;
        $data['step3']['ngay_giao_noinhan'] =  $req->ngay_giao_noinhan;
        $data['step3']['gio_giao_noinhan'] =  $req->gio_giao_noinhan;
        $data['step3']['phut_giao_noinhan'] =  $req->phut_giao_noinhan;
        ...
        // Mã khuyến mãi
        $data['voucher'] = $req->voucher;
         return View('frontend.pages.dat_hang_B3')->with(compact('data','id'));   
    }

    public function step_3_post(Request $req, $id){
        $data['step1'] = [];
        $data['step1']['service_id'] =  $req->service_id;
        $data['step1']['cod_amount'] =  $req->cod_amount;
        $data['step1']['weight'] =  $req->weight;
        ...
        return view('frontend.pages.dat_hang_B4')->with(compact('data','id','result'));
    }
    //kết thúc submit form
}
